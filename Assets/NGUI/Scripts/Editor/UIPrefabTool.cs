...
public class UIPrefabTool : EditorWindow
{
	...
	void Save ()
	{
        ...
        EditTools.SetPTString(mTab, data); 
	}
	...
	void Load ()
	{
        ...
        string data = EditTools.GetPTString(mTab);
	...
	void OnGUI ()
	{
		...
		if (GUILayout.Toggle(newTab == 0, "按钮", "ButtonLeft")) newTab = 0;
		if (GUILayout.Toggle(newTab == 1, "字体", "ButtonMid")) newTab = 1;
		if (GUILayout.Toggle(newTab == 2, "公共控件", "ButtonMid")) newTab = 2;
		if (GUILayout.Toggle(newTab == 3, "界面", "ButtonMid")) newTab = 3;
		if (GUILayout.Toggle(newTab == 4, "其他", "ButtonRight")) newTab = 4;
		GUILayout.EndHorizontal();

        if (mTab == 1) EditorGUILayout.LabelField("选中要修改的label，然后点击对应的字体可直接应用样式");
    ...
    public GameObject AddChild(this GameObject parent) { return AddChild(parent, true); }

    /// <summary>
    /// Add a new child game object.
    /// </summary>

    public GameObject AddChild(this GameObject parent, bool undo)
    {
